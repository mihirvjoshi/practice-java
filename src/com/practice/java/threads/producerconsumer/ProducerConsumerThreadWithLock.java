...
public class ProducerConsumerThreadWithLock {
	...
	// WARNING - to create consumer & producer you must have same producerConsumer instance as below 
	// if you have non static lock & condition variables or your thread will deadlock.
	// The reason is that if you create new ProducerConsumerThreadWithLock() each time to create producer & consumer then
	// lock, hasSpace & hasItems will be created separately for consumer & producer and you will not be able to exchange thread messages
	// like await, signal etc.
	// or another (& most preferred) approach is declare lock, hasSpace & hasItems variables as statick so you dont have to worry about using
	...
	public void run() {
		...
		try {
		    ...
		    System.out.println("producing item number -> " + itemStack.size());
		    hasItems.signalAll();

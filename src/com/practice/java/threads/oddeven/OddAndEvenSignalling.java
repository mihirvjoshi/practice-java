...
public class OddAndEvenSignalling {
	...
	public static void main(String[] args) {
		...
		// odd numbers are removed from the "from" queue --> printed -->
		// incremented --> placed on the "to" queue by one thread
		// "from" queue is "odd number" queue and "to" queue is "even number"
		// queue
		executorService.submit(new TakeAndOfferNext(odds, evens, MAX_NUMBER));
		System.out.println("================================");
		// even numbers are removed from the "from" queue --> printed -->
		// incremented --> placed on the "to" queue by another thread
		// "from" queue is "even number" queue and "to" queue is "odd number"
		// queue

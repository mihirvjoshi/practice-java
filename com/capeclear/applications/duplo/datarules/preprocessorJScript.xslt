...
<!-- 
   Version 2001-04-02
   extended for usage as part of the Java Schematron API
   (problem: Xalan 2.0.0/2.0.1 doesn't handle xsl:namespace-alias
   correct for imported stylesheets)
   The extensions (marked as SCHEMATRON-API) should go into a
   seperate meta-stylesheet.

   exclude-result-prefixes="sch" (line 78) added though it's not
   really necessary for working with the Java API

   Extensions by Anna Hovhannisian, Oliver Becker
-->
...
<xsl:stylesheet version="1.0" 
	...
	xmlns:javascript="http://capeclear.com/javascript"
...
<xsl:template match="sch:schema | schema">
	<axsl:stylesheet version="1.0" 
		...
		<xsl:for-each select="sch:ns | ns">
			...
			<xsl:attribute name="lxslt:yyy" namespace="http://xml.apache.org/xslt"/>
		...
		<xsl:apply-templates select="lxslt:script | script" mode="doScript"/>
	...
	<xsl:template match="lxslt:script | script" mode="doScript">
		<lxslt:component prefix="javascript">
	...
	<xsl:template match="sch:rule[not(@abstract='true')] | rule[not(@abstract='true')]">
		...
		<axsl:template match="{@context}" priority="{4000 - count(preceding-sibling::*)}" mode="M{count(../preceding-sibling::*)}">
			...
			<!-- SCHEMATRON-API begins -->
			<axsl:variable name="contents">
				<!-- Note: the following line belongs to
				     the standard skeleton too -->
				<xsl:apply-templates/>
			</axsl:variable>
			<axsl:if test="normalize-space($contents)">
...
<!-- ============================================================== -->
	...
	<xsl:template name="process-root">
		...
		<!-- unused params: fpi, id, icon, lang, version, schemaVersion -->
		<!-- SCHEMATRON-API: wrap contents into RESULT -->
		<RESULT>
                        <xsl:if test="$title">
                                <TITLE>
	...
	<xsl:template name="process-assert">
		...
		<!-- SCHEMATRON-API: wrap messages into ASSERT -->
		<ASSERT>
	...
	<xsl:template name="process-report">
		...
		<xsl:param name="test"/>
		<!-- unused parameters: id, icon, diagnostics, subject -->
		<!-- SCHEMATRON-API: wrap messages into REPORT -->
		<REPORT>
			<xsl:call-template name="process-message">
				<xsl:with-param name="pattern" select="$test"/>
				<xsl:with-param name="role" select="$role"/>
